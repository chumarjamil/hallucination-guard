"""Unit tests for the Hallucination Risk Scoring engine."""
...
class TestHallucinationScorer:
    ...
    def test_no_claims_returns_zero_risk(self, scorer: HallucinationScorer):
        report = scorer.score([])
        assert report.hallucination_risk == 0.0
        assert report.confidence == 1.0
        assert report.total_claims == 0

    def test_all_supported_low_risk(self, scorer: HallucinationScorer):
    ...
    def test_all_unsupported_high_risk(self, scorer: HallucinationScorer):
        results = [_make_result(False, 0.1) for _ in range(5)]
        report = scorer.score(results)
        assert report.hallucination_risk > 0.7
        assert report.unsupported_claims == 5

    def test_mixed_results(self, scorer: HallucinationScorer):
        results = [
    ...
    def test_risk_in_valid_range(self, scorer: HallucinationScorer):
        results = [_make_result(False, 0.0)]
        report = scorer.score(results)
        assert 0.0 <= report.hallucination_risk <= 1.0
        assert 0.0 <= report.confidence <= 1.0

    def test_report_has_details(self, scorer: HallucinationScorer):
        results = [_make_result(True, 0.8)]
        report = scorer.score(results)
        assert isinstance(report, RiskReport)
        assert len(report.details) == 1

    def test_average_similarity(self, scorer: HallucinationScorer):
